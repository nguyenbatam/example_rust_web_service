...
Parse feed event from JSON string. Uses serde deserialization directly for type safety.
...
**Returns**:
- `Ok((event_type, value))`: Event type (deserialized from JSON) and parsed JSON value
- `Err(e)`: Parse error (missing event_type field or invalid enum value)

**Implementation**:
- Extracts `event_type` field from JSON
- Deserializes directly into `FeedEventType` enum using serde
